...
const updateNumbers = (obj) => {
  ...
  const resultArray = keysArray.map((name) => `${name}: ${obj[name]}`);
...
const getHouses = (arr) => {
  ...
  const houses = arr.map((characters) => characters.house);
  console.log(houses)
  return houses;
};
...
const characterName = arr.find((char) => char.name ==character);

const hasChilden = characterName && 'children' in characterName && Object.value(characterName.children).length > 0;
return hasChilden;
};
